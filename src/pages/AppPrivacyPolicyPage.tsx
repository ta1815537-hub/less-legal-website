    ...
    <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-14 pb-12 sm:pb-16 space-y-8 overflow-hidden bg-slate-50 dark:bg-[#080808] transition-colors duration-300">
      ...
      <div className="space-y-4 relative z-10 text-center sm:text-left">
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/10 dark:bg-white/5 border border-amber-600/30 dark:border-[#D8BD82]/30 text-amber-900 dark:text-[#D8BD82] text-xs font-bold">
          <Smartphone className="w-3.5 h-3.5 text-[#C21F2F] dark:text-[#E03A3E]" />
      ...
      <div className="space-y-4 relative z-10">
        <div className="flex items-center gap-2">
          <Database className="w-5 h-5 text-[#C21F2F] dark:text-[#E03A3E]" />
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-black/40 backdrop-blur-md shadow-xs">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-[#F5F2EE] font-bold border-b border-slate-200 dark:border-white/10">
                <th className="p-3.5 min-w-[150px]">{isHindi ? "डेटा श्रेणी" : "Data Category"}</th>
                <th className="p-3.5 min-w-[100px]">{isHindi ? "संग्रहीत?" : "Collected?"}</th>
                <th className="p-3.5 min-w-[200px]">{isHindi ? "उद्देश्य" : "Purpose"}</th>
                <th className="p-3.5 min-w-[180px]">{isHindi ? "कहाँ प्रोसेस / संग्रहीत" : "Where Processed / Stored"}</th>
                <th className="p-3.5 min-w-[100px]">{isHindi ? "साझा?" : "Shared?"}</th>
                <th className="p-3.5 min-w-[160px]">{isHindi ? "अवधि" : "Retention Period"}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-white/10 text-slate-700 dark:text-[#B8B3AF]">
              {tableData.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                  <td className="p-3.5 font-bold text-slate-900 dark:text-[#F5F2EE]">{row.category}</td>
                  <td className="p-3.5 font-medium">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${row.collected.includes('Yes') || row.collected.includes('हाँ') ? 'bg-amber-500/15 text-amber-800 dark:text-[#D8BD82]' : 'bg-slate-200 dark:bg-white/10 text-slate-700 dark:text-slate-300'}`}>
                      {row.collected}
                    </span>
                  </td>
                  <td className="p-3.5">{row.purpose}</td>
                  <td className="p-3.5">{row.storage}</td>
                  <td className="p-3.5 font-semibold">{row.shared}</td>
                  <td className="p-3.5 text-slate-600 dark:text-[#77736F]">{row.retention}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="space-y-6 text-sm leading-relaxed relative z-10 text-slate-700 dark:text-[#B8B3AF]">
        ...
        <section className="space-y-3 glass-panel p-6 rounded-3xl border border-slate-200 dark:border-white/10">
          ...
          <div className="bg-amber-500/10 dark:bg-white/5 p-4 rounded-2xl border border-amber-600/30 dark:border-white/10 space-y-1">
        ...
        <section className="space-y-3 glass-panel p-6 rounded-3xl border border-slate-200 dark:border-white/10">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex items-start gap-2">
              <HardDrive className="w-4 h-4 text-amber-600 dark:text-[#D8BD82] shrink-0 mt-0.5" />
              <div><strong>{isHindi ? "सीमित ऐप फ़ोल्डर अनुमति (Scoped Scope):" : "Scoped App Folder Permission:"}</strong> {isHindi ? "ऐप केवल अपने निजी ऐप डेटा फ़ोल्डर (drive.appdata) तक पहुँच का अनुरोध करता है। Less Legal आपकी ड्राइव में अन्य निजी फ़ाइलों को कभी भी नहीं देख या बदल सकता है।" : "The app requests scoped access exclusively to its designated hidden app folder (`drive.appdata`). Less Legal CANNOT view, read, or modify unrelated personal files in your Google Drive."}</div>
            </div>
            <div className="flex items-start gap-2">
              <HelpCircle className="w-4 h-4 text-amber-600 dark:text-[#D8BD82] shrink-0 mt-0.5" />
              <div><strong>{isHindi ? "बैकअप रोकना या अनुमति रद्द करना:" : "Stopping & Revoking Backup Access:"}</strong> {isHindi ? "आप ऐप सेटिंग में बैकअप बंद कर सकते हैं या myaccount.google.com/permissions पर जाकर गूगल खाता सुरक्षा से अनुमति रद्द कर सकते हैं।" : "You can disable backup in app settings or revoke access anytime via Google Account Security at myaccount.google.com/permissions."}</div>
            </div>
          </div>
        </section>
      ...
      <div className="pt-6 border-t border-slate-200 dark:border-white/10 flex flex-wrap items-center justify-between gap-4 relative z-10">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#C21F2F] text-white hover:bg-[#8B0000] text-xs font-bold transition-colors cursor-pointer shadow-md"
        >
